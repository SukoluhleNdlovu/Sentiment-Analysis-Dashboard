        ...
        for model in self.models:
            try:
                ...
                response = requests.post(
                    api_url, headers=self.headers, json={"inputs": "test"}, timeout=10
                )
                ...
                with st.spinner("Analyzing..."):
                    ...
                    if result:
                        ...
                        with col1:
                            ...
                            st.markdown(
                                f'<div class="metric-card"><h3>Sentiment</h3><p class="{sentiment_class}">{result["sentiment"].title()}</p></div>',
                                unsafe_allow_html=True,
                            )

                            # Display details
                            st.markdown(
                                create_sentiment_display(result), unsafe_allow_html=True
                            )
...
Amazing customer support!"""
            ...
            if st.button("Analyze Batch", type="primary") and batch_text.strip():
                ...
                if batch_results:
                    ...
                    # Summary
                    positive = sum(
                        1 for r in batch_results if r["sentiment"] == "positive"
                    )
                    negative = sum(
                        1 for r in batch_results if r["sentiment"] == "negative"
                    )
                    neutral = sum(
                        1 for r in batch_results if r["sentiment"] == "neutral"
                    )
                    ...
                    for i, result in enumerate(batch_results):
                        with st.expander(
                            f"Text {i+1}: {result['sentiment'].title()} ({result['confidence']:.1%})"
                        ):
    ...
    with tab3:
        ...
        if st.session_state.results:
            ...
            positive = sum(
                1 for r in st.session_state.results if r["sentiment"] == "positive"
            )
            negative = sum(
                1 for r in st.session_state.results if r["sentiment"] == "negative"
            )
            neutral = sum(
                1 for r in st.session_state.results if r["sentiment"] == "neutral"
            )
            avg_confidence = (
                sum(r["confidence"] for r in st.session_state.results) / total
            )
